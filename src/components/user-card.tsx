...
export function UserCard({ address, name, bio, avatarUrl, price }: UserCardProps) {
    ...
    return (
        <article className="bg-white rounded-xl shadow-sm border border-gray-100 p-5 flex flex-col justify-between hover:shadow-md transition-shadow">
            <Link href={`/u/${address}`} className="flex items-center gap-4 mb-5 group">
                {avatarUrl ? (
                    <Image
                        src={avatarUrl}
                        alt={name}
                    ...
                    <div className="w-[60px] h-[60px] rounded-full bg-gradient-to-br from-blue-500 to-purple-500 flex items-center justify-center text-white text-xl font-bold border border-gray-100">
                        {initials || "?"}
                    </div>
                )}
                <div>
                    ...
                    <p className="text-gray-500 text-sm line-clamp-1">{bio}</p>
                </div>
            </Link>
            <Link
                href={`/u/${address}`}
